...
#define COLLECTD_CPU_STATE_RUNNING 12
#define COLLECTD_CPU_STATE_WAITING 13
#define COLLECTD_CPU_STATE_MAX 14 /* #states */
...
static bool proc_schedstat_available = false; /* currently used for Linux*/
static gauge_t cpu_pressure = 0;
...
static int cpu_config(char const *key, char const *value) /* {{{ */
{
  ...
  else if (strcasecmp(key, "ReportActiveState") == 0)
...
static int cpu_init(void) {
  ...
  notification_t n = {NOTIF_OKAY, cdtime(), "", "", "cpu", "", "", "", NULL};
...
/* nothing to initialize */
  ...
  /* check for availability of scheduling information */
  struct stat statbuf;
  if (stat("/proc/schedstat", &statbuf) == 0) {
    if (S_ISREG(statbuf.st_mode)) {
      proc_schedstat_available = true;
    }
  }
  if (!proc_schedstat_available) {
    WARNING("cpu plugin: no scheduling information available");
  }
#endif
 ...
 * aggregation. rates is a pointer to COLLECTD_CPU_STATE_RUNNING gauge_t values
 * holding the
 * current rate; each rate may be NAN. Calculates the percentage of each state
 * and dispatches the metric. */
static void cpu_commit_one(int cpu_num, /* {{{ */
                           gauge_t rates[static COLLECTD_CPU_STATE_RUNNING]) {
  gauge_t sum;
  ...
  size_t max_state = COLLECTD_CPU_STATE_ACTIVE;
  if (report_active)
    ++max_state;
...
{
  gauge_t global_rates[COLLECTD_CPU_STATE_RUNNING] = {
  ...
  for (size_t cpu_num = 0; cpu_num < global_cpu_num; cpu_num++) {
    cpu_state_t *this_cpu_states = get_cpu_state(cpu_num, 0);
    gauge_t local_rates[COLLECTD_CPU_STATE_RUNNING] = {
...
int value_to_rate_pressure(gauge_t *ret_rate, /* {{{ */
                           value_t value, value_to_rate_state_t *state) {

  /* Previous value is invalid. */
  if (state->last_value.derive == 0) {
    state->last_value = value;
    return EAGAIN;
  }
  derive_t diff = value.derive - state->last_value.derive;
  *ret_rate = (gauge_t)diff;
                     ...
                     cdtime_t now) /* {{{ */
{
  ...
  if (state == COLLECTD_CPU_STATE_ACTIVE || state >= COLLECTD_CPU_STATE_MAX)
    return EINVAL;
  ...
  if (state == COLLECTD_CPU_STATE_RUNNING ||
      state == COLLECTD_CPU_STATE_WAITING)
    status = value_to_rate_pressure(&rate, val, &s->conv);
  else
...
static void cpu_notify(cdtime_t now) {
  const cpu_state_t *s;
  gauge_t core_pressure = NAN;
  notification_t n = {NOTIF_OKAY, now, "", "", "cpu", "", "cpu_core", "", NULL};

  for (size_t cpu_num = 0; cpu_num < global_cpu_num; cpu_num++) {

    gauge_t percentages[COLLECTD_CPU_STATE_MAX] = {
        NAN, NAN, NAN, NAN, NAN, NAN, NAN, NAN, NAN, NAN, NAN, NAN, NAN, NAN};

    bool anything_to_report = false;
    bool waiting_to_report = false;
    bool running_to_report = false;

    gauge_t total = get_cpu_state(cpu_num, COLLECTD_CPU_STATE_ACTIVE)->rate;
    RATE_ADD(total, get_cpu_state(cpu_num, COLLECTD_CPU_STATE_IDLE)->rate);

    for (size_t state = 0; state <= COLLECTD_CPU_STATE_ACTIVE; state++) {
      s = get_cpu_state(cpu_num, state);
      if (s->has_value) {
        percentages[state] = 100.0 * s->rate / total;
        anything_to_report = true;
      }
    }

    if (proc_schedstat_available) {
      s = get_cpu_state(cpu_num, COLLECTD_CPU_STATE_RUNNING);
      if (s->has_value) {
        percentages[COLLECTD_CPU_STATE_RUNNING] = s->rate;
        running_to_report = true;
      }
      s = get_cpu_state(cpu_num, COLLECTD_CPU_STATE_WAITING);
      if (s->has_value) {
        percentages[COLLECTD_CPU_STATE_WAITING] = s->rate;
        waiting_to_report = true;
      }
    }

    if (waiting_to_report && running_to_report) {
      core_pressure = percentages[COLLECTD_CPU_STATE_WAITING] /
                      percentages[COLLECTD_CPU_STATE_RUNNING];
      anything_to_report = true;
    }
    ...
    ssnprintf(n.message, sizeof(n.message),
              "Proc nr. %zu; idle = %.2f; iowait = %.2f; irq = %.2f; nice = "
              "%.2f; softirq = %.2f; steal = %.2f; system = %.2f; user = %.2f; "
              "pressure = %.2f; ts = %ju",
              cpu_num, (double)percentages[COLLECTD_CPU_STATE_IDLE],
              percentages[COLLECTD_CPU_STATE_WAIT],
              percentages[COLLECTD_CPU_STATE_INTERRUPT],
              percentages[COLLECTD_CPU_STATE_NICE],
              percentages[COLLECTD_CPU_STATE_SOFTIRQ],
              percentages[COLLECTD_CPU_STATE_STEAL],
              percentages[COLLECTD_CPU_STATE_SYSTEM],
              percentages[COLLECTD_CPU_STATE_USER], core_pressure,
              (uintmax_t)CDTIME_T_TO_TIME_T(now));
  ...
  if (report_cpu_pressure) {
    notification_t p = {NOTIF_OKAY,     now, "",  "", "cpu", "",
                        "cpu_pressure", "",  NULL};

    snprintf(p.message, sizeof(p.message), "CPU pressure %s: %.2f", hostname_g,
             cpu_pressure);
...
static int cpu_read(void) {
  ...
  if (proc_schedstat_available) {
    if ((fh = fopen("/proc/schedstat", "r")) == NULL) {
      ERROR("cpu plugin: fopen (/proc/schedstat) failed: %s", STRERRNO);
      return -1;
    }
    memset(buf, 0, 1024);
    while (fgets(buf, 1024, fh) != NULL) {
      ...
      cpu = atoi(fields[0] + 3);
      /* cpu running and waiting time*/
      cpu_stage(cpu, COLLECTD_CPU_STATE_RUNNING, (derive_t)atoll(fields[7]),
                now);
      cpu_stage(cpu, COLLECTD_CPU_STATE_WAITING, (derive_t)atoll(fields[8]),
                now);
    }
    fclose(fh);
  }
  ...
  if (report_cpu_pressure) {
    if ((fh = fopen("/proc/pressure/cpu", "r")) == NULL) {
      WARNING("cpu plugin: fopen (/proc/pressure/cpu) failed: %s", STRERRNO);
      return -1;
    }
    ...
    while (fgets(buf, sizeof(buf), fh) != NULL) {
      if (strncasecmp(buf, "some", 4) == 0) {
        ...
        if (numfields < 2)
          continue;
        // extracting cpu pressure value from the part after "some="
        cpu_pressure = atof(fields[1] + 6);
        // converting value to match requirement as mentioned in APRICOT-517570
        cpu_pressure = cpu_pressure / 100;
      }
    }
    if (fclose(fh)) {
      WARNING("cpu: fclose: %s", STRERRNO);
    }
  }
